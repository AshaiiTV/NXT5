...
export function publicationFixture({ timeline = true, longNames = false, incomplete = false, side = 'blue' } = {}) {
  ...
  const participants = champions.map((champion, index) => ({
    ...
    assists: index < 5 ? 8 + index : 5 + index, gold: (index < 5 ? 13000 : 12000) + (index % 5) * 125,
    damage: 19000 + index * 750, vision: 17 + index * 5,
    raw: { participantId: index + 1, teamId: (index < 5) === (side === 'blue') ? 100 : 200 },
  }));
  const match = { id: 'example-match', team_id: 'example-team', game_id: 'EXEMPLE_2026', result: 'Victoire', side: side.toUpperCase(), duration: '31:42', category_ids: ['scrims'], opponent: longNames ? 'Adversaires de démonstration avec un nom de groupe très long' : 'Équipe démo B', participants,
