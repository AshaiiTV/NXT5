...
async function handler(request: Request, context: Context) {
  try {
    ...
    if (!validBotToken(token)) throw discordError('Ce lien personnel est invalide. Lance /nxt compte lier dans Discord.');
    ...
    if (!pending || (pending.user_id && pending.user_id !== user.id)) throw discordError('Ce lien a expiré ou est déjà utilisé. Lance /nxt compte lier dans Discord.', 409, 'DISCORD_ACCOUNT_LINK_EXPIRED');
    ...
    if (request.method === 'POST') {
      ...
      if (!changed.length) throw discordError('La demande a changé. Lance /nxt compte lier dans Discord.', 409, 'DISCORD_ACCOUNT_LINK_EXPIRED');
    }
