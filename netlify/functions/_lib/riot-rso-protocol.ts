...
const CALLBACK_PATHS = [CALLBACK_PATH, '/.netlify/functions/auth-social-callback'];
...
export function getRiotRsoConfig(): RiotRsoConfig | null {
  if (process.env.RIOT_RSO_ENABLED !== 'true' || process.env.RIOT_RSO_APPROVAL_CONFIRMED !== 'true') return null;
  try {
    const siteOrigin = process.env.RIOT_RSO_SITE_ORIGIN || 'https://nxt5.org';
    const origin = new URL(siteOrigin);
    if (origin.protocol !== 'https:' || origin.origin !== siteOrigin || origin.username || origin.password) return null;
    const redirectUri = process.env.RIOT_RSO_REDIRECT_URI || `${siteOrigin}${CALLBACK_PATH}`;
    const postLogoutRedirectUri = process.env.RIOT_RSO_POST_LOGOUT_REDIRECT_URI || `${siteOrigin}${POST_LOGOUT_PATH}`;
    if (!CALLBACK_PATHS.some(path => exactHttpsUrl(redirectUri, siteOrigin, path))
      || !exactHttpsUrl(postLogoutRedirectUri, siteOrigin, POST_LOGOUT_PATH)) return null;
    const clientId = process.env.RIOT_RSO_CLIENT_ID || '';
    if (!clientId || clientId.length > 256 || /[\s\u0000-\u001f\u007f]/.test(clientId)) return null;
    const accountRegion = process.env.RIOT_RSO_ACCOUNT_REGION || 'europe';
    if (!['europe', 'americas', 'asia'].includes(accountRegion)) return null;
    let auth: RiotRsoConfig['auth'];
    if (process.env.RIOT_RSO_CLIENT_AUTH_METHOD === 'client_secret_basic') {
    ...
    } else if (process.env.RIOT_RSO_CLIENT_AUTH_METHOD === 'private_key_jwt') {
      const keyId = process.env.RIOT_RSO_PRIVATE_KEY_ID || '';
      ...
      const assertionAudience = process.env.RIOT_RSO_PRIVATE_KEY_AUDIENCE || '';
      if (![ISSUER, TOKEN_URL].includes(assertionAudience)) return null;
      const pem = (process.env.RIOT_RSO_PRIVATE_KEY || '').replace(/\\n/g, '\n');
