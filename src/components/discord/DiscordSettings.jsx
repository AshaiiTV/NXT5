...
function DiscordSettingsContent({ teamId, teamName, canManage, canPublish }) {
  ...
  return <Surface className="discord-panel discord-dashboard">
    ...
    {status?.health?.checkedAt && <p className="discord-help discord-last-checked">Dernière vérification : {new Date(status.health.checkedAt).toLocaleString("fr-FR")}.</p>}
    ...
    {status?.configured && <>
      <section className="discord-section discord-configuration" aria-label="Configuration du bot">
        <h4>{established ? "Configuration" : "Connecter ton équipe en quatre étapes"}</h4>
        <p className="discord-configuration-intro">{established ? "Choisis une rubrique pour consulter ou modifier sa configuration." : "Choisis une étape pour afficher son contenu. La première connexion reste en pause jusqu’à ton activation."}</p>
        {progress}
        <div className="discord-step-panels">
          ...
          <section id={`${stepsId}-panel-1`} role="tabpanel" aria-labelledby={`${stepsId}-tab-1`} hidden={activeStep !== 1} tabIndex={0} className="discord-step-panel discord-setup-step">
            ...
            {connected ? <><Badge tone="cyan">Équipe et serveur associés</Badge><p><strong>{teamName || "Ton équipe"}</strong> est reliée à <strong>{connection.guildName || connection.guildId}</strong>. Les salons et catégories de cette équipe se règlent à l’étape suivante, indépendamment des autres équipes du serveur.</p><Button type="button" variant="primary" onClick={() => selectStep(2, true)}>Choisir les destinations</Button></> : <>
...
function DiscordStepPrerequisite({ title, onLink }) {
  return <div className="discord-setup-step"><h4>{title}</h4><p>Relie d’abord ton équipe à son serveur Discord. Ses salons seront ensuite disponibles pour préparer et tester la diffusion.</p><Button type="button" variant="primary" icon={Link2} onClick={onLink}>Aller à Relier</Button></div>;
}
...
function DiscordRoutesEditor({ teamId, metadata, channelsLoading, channelsError, routesLoading, routesError, canManage, onSaved, onDirty, initialRoutes }) {
  ...
  return <form className="discord-section" onSubmit={save}>
    ...
    <DiscordFeedback error={action.error} notice={action.notice} />
    {canManage && <div className="discord-actions discord-save-actions"><Button type="submit" variant={dirty ? "primary" : "ghost"} icon={action.busy ? Loader2 : Check} disabled={action.busy || !dirty || !canSave}>Enregistrer les destinations</Button>{dirty && <p className="discord-help" role="status">Modifications non enregistrées.</p>}</div>}
  </form>;
}
