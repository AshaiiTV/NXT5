...
function DiscordRoutesEditor({ teamId, metadata, channelsLoading, channelsError, routesLoading, routesError, canManage, onSaved, onDirty, initialRoutes }) {
  ...
  return <form className="discord-section" onSubmit={save}>
    <span className="discord-step-kicker">Publication des parties</span><h4>Salons de publication</h4>
    <p>Ces destinations servent à publier les parties. Le salon des commandes ci-dessus détermine l’équipe lorsqu’un joueur utilise le bot.</p>
    ...
    <div className="discord-routes">{routes.map((route, index) => {
      ...
      return <fieldset key={route.id || `new-${index}`} className="discord-route" disabled={!canManage || action.busy}>
        ...
        <label className="discord-check"><input type="checkbox" checked={route.enabled} onChange={(event) => patch(index, { enabled: event.target.checked })} />Diffuser automatiquement les nouvelles parties correspondantes</label>
