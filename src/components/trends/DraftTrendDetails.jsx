...
const SECTION_CRITERIA = {
  "pick-repere": "Le pick repère est le pick de confort le plus joué : au moins 2 games et 50% de victoires. Si aucun pick ne remplit ces critères, le pick le plus joué sert de repère. À nombre de games égal, le taux de victoire départage les picks.",
  confort: "Un pick de confort associe un champion et un rôle, avec au moins 2 games et 50% de victoires sur la période sélectionnée. Tous les picks qui remplissent ces critères sont affichés ici.",
  ...
  compositions: "Chaque draft est classée selon le marqueur de style le plus présent parmi ses champions. Une identité décrit les possibilités de la composition ; elle ne mesure pas la manière dont la game a été jouée.",
  duos: "Un duo associe deux champions joués ensemble dans une même game à une paire de rôles donnée. Trois paires sont suivies : Jungle + Mid, ADC + Support et Top + Jungle. Les autres associations de rôles ne sont pas comptabilisées.",
  "a-revoir": "Les picks à revoir ont été joués au moins 2 fois avec moins de 50% de victoires sur la période. Les signaux de style sont des pistes de review : ouvre les games sources pour examiner le contexte et les décisions de l’équipe.",
  roles: "Chaque champion est compté séparément au rôle auquel il a été joué. Tous les picks sont disponibles ici, y compris ceux joués une seule fois. La recherche et le filtre de rôle permettent de comparer leur utilisation.",
};
...
function DraftTrendDetails({ sectionId, model, onOpenSources, sourceGamesForMatches }) {
  ...
  if (!active?.games) return <Surface className="draft-trends-surface"><div className="draft-trends-content"><p className="draft-empty">Aucune donnée de draft sur cette sélection. Élargis la période ou importe des games avec leurs participants.</p></div></Surface>;

  const openSources = (entry, title, subtitle) => onOpenSources?.({
    title,
    subtitle,
    metrics: [
      { label: "Games", value: String(entry.games) },
  ...
  const sourceAction = (pick) => onOpenSources ? () => openSources(pick, `Pick équipe : ${championDisplayName(pick.champion)}`, `${roleLabel(pick.role)} · ${pick.games} games · ${pick.wr}% de victoires`) : undefined;
  const tableSources = onOpenSources ? (row) => openSources(row, row.label || (row.tag ? `Composition équipe : ${tagLabel(row.tag)}` : row.champions), `${row.pair ? `${row.pair} · ` : ""}${row.games} games · ${row.wr}% de victoires`) : undefined;
  ...
  return <Surface className="draft-trends-surface draft-detail-surface">
    <div className="draft-trends-content draft-detail-content">
      <div className="draft-detail-intro">
        ...
        <p className="draft-footnote">Fréquence = nombre de games {sectionId === "profil" ? "avec au moins un marqueur de cette famille" : "avec ce pick, cette composition ou ce duo"} / {active.games} drafts disponibles. Le taux de victoire (WR) porte uniquement sur les games de la ligne. V = victoires, D = défaites. Ces résultats décrivent cet échantillon, sans établir de causalité.</p>
      </div>

      {sectionId === "pick-repere" && mainPick && <section className="draft-detail-section">
        <h3>Le repère de cette période</h3>
        {pickList([mainPick])}
        <p className="draft-footnote">KDA = (kills + assists) / morts, calculé sur l’ensemble des games du pick ; le diviseur vaut 1 si aucune mort n’est enregistrée.</p>
      </section>}
      ...
      {sectionId === "a-revoir" && <section className="draft-detail-section">
        ...
        <p className="draft-footnote">Les signaux d’initiation, de frontline et de contrôle signalent l’absence de leurs marqueurs dans les picks du bloc. Les signaux de late game ou de tempo apparaissent à partir de trois marqueurs correspondants dans le bloc.</p>
        {onOpenSources && <button type="button" className="draft-source-link" onClick={() => openSources({ games: active.games, wins: active.wins, wr: active.wr, matches: active.matchDrafts.map((entry) => entry.match) }, "Drafts à examiner", "Ensemble des drafts de la période pour remettre les signaux en contexte.")}>Examiner les games du bloc <ArrowRight aria-hidden="true" /></button>}
      </section>}

      {PICK_SECTIONS.has(sectionId) && <section className="draft-detail-section">
        <h3>{sectionId === "confort" ? "Tous les picks de confort" : sectionId === "a-revoir" ? "Tous les picks à revoir" : sectionId === "roles" ? "Le pool complet par rôle" : "Tous les picks de la période"}</h3>
        <p className="draft-description">{sectionId === "a-revoir" ? "Tri par taux de victoire croissant, puis par nombre de games." : "Tri par nombre de games, puis par taux de victoire."} Un même champion joué à plusieurs rôles apparaît sur plusieurs lignes.</p>
        <DetailFilters query={query} setQuery={setQuery} searchLabel="Rechercher un champion" filter={role} setFilter={setRole} filterLabel="Rôle" options={availableRoles.map((value) => ({ value, label: roleLabel(value) }))} count={visiblePicks.length} total={eligiblePicks.length} noun="picks" />
        {visiblePicks.length ? sectionId === "roles" ? availableRoles.map((pickRole) => {
          ...
          return <section className="draft-detail-role" key={pickRole}><h4 className="draft-detail-role-heading"><span aria-hidden="true"><RoleIcon role={pickRole} className="draft-role-icon" lightweight /></span><span>{roleLabel(pickRole)}</span><span className="draft-context">{rolePicks.length} pick{rolePicks.length > 1 ? "s" : ""}</span></h4>{pickList(rolePicks)}</section>;
        }) : pickList(visiblePicks) : <p className="draft-empty">{eligiblePicks.length ? "Aucun pick ne correspond à cette recherche. Modifie le rôle ou le nom recherché." : "Aucun pick ne remplit ces critères sur cette sélection. Élargis la période pour retrouver davantage de games."}</p>}
        {onOpenSources && visiblePicks.length > 0 && <p className="draft-footnote">Sélectionne un champion pour consulter ses games sources.</p>}
      </section>}

      {sectionId === "duos" && <section className="draft-detail-section">
        <h3>Tous les duos de la période</h3>
        <p className="draft-description">Tri par nombre de games, puis par taux de victoire. Une game peut contribuer à trois duos. Un duo n’est compté que si les deux rôles sont renseignés.</p>
      ...
      {sectionId === "compositions" && <section className="draft-detail-section">
        ...
        <p className="draft-description">Tri par nombre de games, puis par taux de victoire. « Standard » indique qu’aucun marqueur plus spécifique n’a été identifié.</p>
        ...
        <p className="draft-footnote">Deux compositions avec des champions différents peuvent partager la même identité. Ouvre les sources pour consulter les picks de chaque game.</p>
      </section>}
      ...
      {sectionId === "profil" && <>
        ...
        <section className="draft-detail-section"><h3>Comment les familles sont définies</h3><dl className="draft-detail-definitions">{DRAFT_SCORE_TAGS.map(([id, tags]) => <div key={id}><dt>{tagLabel(id)}</dt><dd>{tags.map(tagLabel).join(" · ")}</dd></div>)}</dl><p className="draft-footnote">Ces familles utilisent les marqueurs de champions connus par NXT5. Les champions sans marqueurs spécifiques restent classés « Standard » ; leurs possibilités ne sont pas déduites du résultat de la game.</p></section>
      </>}
    </div>
  </Surface>;
}
