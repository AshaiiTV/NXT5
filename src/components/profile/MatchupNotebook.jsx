...
export function ChampionSectionTabs({ active, onChange, id }) {
  ...
  return <div className="matchup-section-tabs nxt5-tab-nav" role="tablist" aria-label="Rubriques du champion">
    {tabs.map((tab, index) => <button type="button" className="nxt5-tab" role="tab" key={tab.id} id={id + "-" + tab.id} aria-controls={id + "-panel"} aria-selected={active === tab.id} tabIndex={active === tab.id ? 0 : -1} onClick={() => onChange(tab.id)} onKeyDown={(event) => selectWithKey(event, index)}>{tab.id === "matchups" && <BookOpen size={16} aria-hidden="true" />}{tab.label}</button>)}
  </div>;
}
  ...
  return <section className="matchup-notebook" ref={root}>
    {selected ? <>
      ...
      <header className="matchup-heading">
        ...
        <div><h4 ref={heading} tabIndex={-1}>{championDisplayName(champion)} face à {championDisplayName(selected.opponentChampion)}</h4><p>{roleLabel(selected.role)} · {selected.games} partie{selected.games > 1 ? "s" : ""} dans la catégorie sélectionnée</p></div>
      </header>
      ...
      <header className="matchup-heading"><div><h4>Carnets de matchups</h4><p>Prépare chaque duel (matchup), puis garde le plan et les conclusions de l’équipe.</p></div></header>
      ...
      <p className="matchup-meta" role="status">{visible.length} matchup{visible.length > 1 ? "s" : ""} · les victoires concernent les parties entières</p>
      <div className="matchup-list">{visible.map((group) => {
        ...
        return <button type="button" key={group.key} data-matchup={group.key} className="matchup-row" onClick={() => setSelectedKey(group.key)}>
          ...
          <span><strong>{group.games}</strong><small>partie{group.games > 1 ? "s" : ""}</small></span>
          <span><strong>{number(group.results.rate)}{group.results.rate === null ? "" : " %"}</strong><small>Victoires · {group.results.count} résultat{group.results.count > 1 ? "s" : ""} connu{group.results.count > 1 ? "s" : ""}</small></span>
          <span><strong>{signed(group.cs10.value)}{group.cs10.value === null ? "" : " CS"}</strong><small>Écart à 10 min · {group.cs10.count} partie{group.cs10.count > 1 ? "s" : ""}</small></span>
      ...
      {visible.length > 0 && <p className="matchup-meta matchup-list-help">CS : sbires et monstres tués. L’écart est calculé face à l’adversaire au même poste.</p>}
      {!visible.length && <p className="matchup-notice">{query ? "Aucun matchup ne correspond à cette recherche." : "Les matchups apparaissent avec les parties importées et un adversaire identifié au même poste."}</p>}
      {excluded > 0 && <p className="matchup-meta">{excluded} partie{excluded > 1 ? "s" : ""} sans adversaire de même poste identifié sans ambiguïté. Vérifie les postes dans Parties pour les retrouver ici.</p>}
    </>}
  </section>;
}
...
function NotebookContent({ draftKey, group, notebook, canEdit, onSave, onReload, navigate }) {
  ...
  return <section className="matchup-content">
    <div className="matchup-section-heading"><div><h5>Plan de départ et essais</h5><p className="matchup-meta">{notebook?.updatedAt ? "Mis à jour le " + date(notebook.updatedAt) + (notebook.updatedByName ? " par " + notebook.updatedByName : "") : "Prépare la prochaine rencontre."}</p></div>
    ...
    {editingNow ? <form id={formId} onSubmit={async (event) => { event.preventDefault(); if (await draft.submit()) setEditing(false); }}>
      <fieldset disabled={draft.busy} className="matchup-fields">
        ...
        {!current.experiments.length && <p className="matchup-meta">Ajoute une approche à tester, puis associe les parties et tes observations.</p>}
      ...
      {current.experiments.length ? current.experiments.map((experiment) => <details className="matchup-experiment" key={experiment.id}>
        <summary><strong>{experiment.title}</strong><Badge tone={experiment.status === "active" ? "purple" : "cyan"}>{EXPERIMENT_STATES[experiment.status]}</Badge><span className="matchup-meta">{experiment.matchIds.length} partie{experiment.matchIds.length > 1 ? "s associées" : " associée"}</span></summary>
...
function LinkedGames({ ids, rows, navigate }) {
  if (!ids.length) return <p className="matchup-meta">Aucune partie associée.</p>;
  return <ul className="matchup-linked-games">{ids.map((id) => {
    ...
    return <li key={id}><a href={href} onClick={(event) => { if (navigate && !event.metaKey && !event.ctrlKey && !event.shiftKey && !event.altKey && event.button === 0) { event.preventDefault(); navigate(href); } }}>{match ? matchDisplayName(match) : "Partie hors du périmètre actuel"}<ArrowRight size={14} aria-hidden="true" /></a></li>;
  })}</ul>;
}
...
function ExperimentEditor({ experiment, index, rows, onChange, onRemove }) {
  ...
  experiment.matchIds.forEach((id) => { if (!choices.has(id)) choices.set(id, "Partie hors de la catégorie actuelle"); });
  return <fieldset className="matchup-experiment-editor">
    ...
    <details className="matchup-game-picker"><summary>Associer des parties <span>· {experiment.matchIds.length} sélectionnée{experiment.matchIds.length > 1 ? "s" : ""}</span></summary><p className="matchup-meta">Choisis les parties qui ont servi à cet essai, dans la catégorie actuelle. Les associations précédentes sont conservées.</p>
      {choices.size ? <div>{[...choices.entries()].map(([id, label]) => <label key={id}><input type="checkbox" checked={experiment.matchIds.includes(id)} disabled={!experiment.matchIds.includes(id) && experiment.matchIds.length >= 50} onChange={(event) => onChange({ matchIds: event.target.checked ? [...experiment.matchIds, id] : experiment.matchIds.filter((matchId) => matchId !== id) })} /><span>{label}</span></label>)}</div> : <p className="matchup-meta">Aucune partie à associer dans cette catégorie.</p>}
    </details>
...
function MatchupStatistics({ group, teamId, bootstrapRevision, renderGames }) {
  ...
  return <section className="matchup-statistics">
    <div className="matchup-section-heading"><div><h5>Statistiques du duel</h5><p className="matchup-meta">Écart moyen du joueur avec l’adversaire au même poste. Chaque mesure indique le nombre de parties renseignées.</p></div><SelectInput label="Patch des statistiques" value={patch} onChange={setPatch}><option value="">Tous les patches</option>{patches.map((value) => <option key={value} value={value}>{value}</option>)}</SelectInput></div>
    <p>{rows.length} partie{rows.length > 1 ? "s" : ""} · {stats.results.rate === null ? "Résultats indisponibles" : number(stats.results.rate) + " % de victoires"} <span className="matchup-meta">{stats.results.count} résultat{stats.results.count > 1 ? "s" : ""} connu{stats.results.count > 1 ? "s" : ""} · la victoire ne mesure pas à elle seule la réussite du duel.</span></p>
    {details.loading && <p className="matchup-notice" role="status">Chargement des relevés du duel… {details.loaded}/{details.total} parties</p>}
    ...
    <details className="matchup-statistics-details"><summary>Relevés à 10, 15 et 20 minutes<span>CS, or et expérience face à l’adversaire</span></summary>
    <p className="matchup-meta">CS : sbires et monstres tués · PO : pièces d’or · XP : expérience.</p>
    <div className="matchup-milestones">{stats.milestones.map((milestone) => <section key={milestone.minute}><h6>À {milestone.minute} minutes</h6><dl>{[["cs", "Écart de CS", " CS"], ["gold", "Écart d’or", " PO"], ["xp", "Écart d’expérience", " XP"]].map(([field, label, unit]) => {
      ...
      return <div key={field}><dt>{label}</dt><dd className={metric.value > 0 ? "matchup-positive" : metric.value < 0 ? "matchup-negative" : ""}>{signed(metric.value)}{metric.value === null ? "" : unit}<small>{metric.count}/{rows.length} parties renseignées</small></dd></div>;
    })}</dl></section>)}</div>
    <p className="matchup-meta">« — » : donnée indisponible. Les relevés peuvent être décalés d’au plus une minute. Les changements de poste pendant la partie restent à vérifier en débrief.</p>
    </details>
    <section className="matchup-source-games"><h5>Parties, runes et équipements</h5><p className="matchup-meta">Ouvre une partie pour comparer les runes, les compétences et les achats des deux joueurs.</p>{renderGames?.(rows)}</section>
  </section>;
}
