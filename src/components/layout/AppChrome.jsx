...
export function Sidebar({ active, setActive, open, setOpen, collapsed, setCollapsed, user, onLogout, currentTeam, currentMember, linkedPlayer, roleLabel, isPlatformAdmin = false }) {
  ...
  return (
    <>
      ...
      <aside className={cx("nxt5-sidebar fixed left-0 top-0 z-40 flex h-screen flex-col border-r border-cyan-200/18 bg-[#050917]/90 p-3 text-white shadow-2xl shadow-black/50 backdrop-blur-2xl transition-all duration-300 lg:translate-x-0", collapsed ? "is-collapsed lg:w-24" : "lg:w-64", open ? "translate-x-0 w-[19rem] max-w-[calc(100vw-1rem)]" : "-translate-x-full w-[19rem] max-w-[calc(100vw-1rem)]")}>
        ...
        <nav className="relative z-10 flex-1 space-y-1.5 overflow-y-auto pr-1">
          ...
          {navItems.map((item) => {
            const Icon = item.id === "profile" ? ProfilePictogram : item.icon;
