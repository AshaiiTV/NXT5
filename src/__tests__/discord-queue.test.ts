...
describe('Discord command replay and server linkage against PostgreSQL',() => {
  const commandChannel='100000000000000002';
  const otherCommandChannel='100000000000000003';
  const interaction=(id:string,name:string,code?:string,channelId=commandChannel)=>({id,guild_id:'100000000000000001',channel_id:channelId,member:{user:{id:'100000000000000050'},permissions:'32'},data:{options:[{name,options:code ? [{name:'code',value:code}]:[]}]}});
  ...
  async function addSharedTeam(name='Academy',guild='100000000000000001',role:'captain'|'player'|null='captain') {
    ...
    await database.pg.query("insert into discord_connections(team_id,guild_id,command_channel_id,status,enabled_at) values($1,$2,$3,'active',now()-interval '1 hour')",[otherTeam,guild,otherCommandChannel]);
  ...
  it('links two teams concurrently to the same server and consumes their own codes once',async () => {
    ...
    await database.pg.exec("update discord_connections set guild_id=null,command_channel_id=null,status='pending',enabled_at=null");
  ...
  it.each(['pause','reprendre','statut'])('rejects %s outside the salon associated with a team',async name => {
    await addSharedTeam();
    const before=await rows('select team_id,status,config_version from discord_connections order by team_id');
    const response=await executeDiscordCommand(interaction('100000000000000120',name,undefined,'100000000000000099'));
    expect(response).not.toContain('NXT5 test');
    expect(response).not.toContain('Academy');
  ...
  it('pauses and resumes only the team associated with the current salon',async () => {
    await addSharedTeam();
    expect(await executeDiscordCommand(interaction('100000000000000121','pause'))).toContain('NXT5 test');
    ...
    expect(await executeDiscordCommand(interaction('100000000000000122','reprendre'))).toContain('Connexion active pour NXT5 test');
    ...
    expect(await executeDiscordCommand(interaction('100000000000000123','statut',undefined,otherCommandChannel))).toContain('Academy : en pause');
  ...
  it('rejects salons associated with another server or a disconnected team',async () => {
    await addSharedTeam('Foreign','100000000000000099');
    expect(await executeDiscordCommand(interaction('100000000000000124','pause',undefined,otherCommandChannel))).not.toContain('Foreign');
    ...
    expect(await executeDiscordCommand(interaction('100000000000000125','reprendre',undefined,otherCommandChannel))).not.toContain('Foreign');
    expect(await rows('select status from discord_connections where team_id=$1',[teamId])).toEqual([{status:'active'}]);
    expect(await rows("select action from audit_logs where action in ('discord.pause','discord.resume')")).toEqual([]);
  });
  it('does not let a legacy equipe option override the team bound to the salon',async () => {
    await addSharedTeam();
    const command=interaction('100000000000000126','pause');
    command.data.options[0].options=[{name:'equipe',value:otherTeam}];
    expect(await executeDiscordCommand(command)).toBe(accessDenied);
    expect(await rows('select team_id,status from discord_connections order by team_id')).toEqual([{team_id:teamId,status:'active'},{team_id:otherTeam,status:'active'}]);
  });
  it('distinguishes teams with the same name by their salons',async () => {
    await addSharedTeam('NXT5 test');
    expect(await executeDiscordCommand(interaction('100000000000000127','pause',undefined,otherCommandChannel))).toContain('NXT5 test');
    expect(await rows('select team_id,status from discord_connections order by team_id')).toEqual([{team_id:teamId,status:'active'},{team_id:otherTeam,status:'paused'}]);
    expect(await executeDiscordCommand(interaction('100000000000000128','statut'))).toContain('NXT5 test : active');
  });
  it('escapes the label of the team resolved from the salon',async () => {
    await addSharedTeam('[Academy](https://example.test)');
    const result=await executeDiscordCommand(interaction('100000000000000130','pause',undefined,otherCommandChannel));
    expect(result).toContain('\\[Academy\\]\\(https://example.test\\)');
    expect(await rows('select team_id,status from discord_connections order by team_id')).toEqual([{team_id:teamId,status:'active'},{team_id:otherTeam,status:'paused'}]);
  });
  it('keeps another shared team inaccessible even for a Discord server manager',async () => {
    ...
    for(const [index,name] of ['statut','pause','reprendre'].entries()) {
      expect(await executeDiscordCommand(interaction(`10000000000000014${index}`,name,undefined,otherCommandChannel))).toBe(accessDenied);
    }
    ...
    expect(await executeDiscordCommand(interaction('100000000000000145','pause',undefined,otherCommandChannel))).toBe(accessDenied);
  });
});
