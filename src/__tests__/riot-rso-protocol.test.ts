...
describe('RSO configuration stays inactive until explicit approval and valid server configuration', () => {
  it('reads runtime Netlify variables before local process defaults', () => {
    vi.stubGlobal('Netlify', { env: { get: (name: string) => name === 'RIOT_RSO_ENABLED' ? 'false' : undefined } });
    expect(getRiotRsoConfig()).toBeNull();
    vi.stubGlobal('Netlify', { env: { get: (name: string) => name === 'RIOT_RSO_CLIENT_ID' ? 'runtime-client' : undefined } });
    expect(requireRiotRsoConfig().clientId).toBe('runtime-client');
  });
