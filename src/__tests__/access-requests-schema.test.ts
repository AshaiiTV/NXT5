...
describe('access request schema readiness', () => {
  it('requires the Structure migration and recovers when it becomes available', async () => {
    query.mockResolvedValueOnce([]).mockResolvedValueOnce([{ migration_key: 'pricing-access-requests-structure-20260908-v1' }]);
    ...
    expect(query.mock.calls[0][1]).toBe('pricing-access-requests-structure-20260908-v1');
  });

  it('checks readiness once for concurrent requests without runtime DDL', async () => {
    query.mockResolvedValue([{ migration_key: 'pricing-access-requests-structure-20260908-v1' }]);
    const { ensureAccessRequestsSchema } = await import('../../netlify/functions/_lib/schema');
