...
const channelA = '200000000000000006', channelB = '200000000000000007';
...
describe('New bot workflows deny foreign-team access on a shared server', () => {
  ...
  it('does not turn staff status on Team A into mutation rights on Team B via a foreign object ID', async () => {
    ...
    for(const [path,options] of attempts){
      ...
      expect(description(result),path).not.toContain('Le résultat n’a pas pu être confirmé');
  ...
  it('rejects forged team options and callbacks targeting foreign-team events and reviews', async () => {
    ...
    for(const attempt of attempts){
      ...
      expect(description(result)).not.toContain('Le résultat n’a pas pu être confirmé');
  ...
  it('pins confirmations to their original salon and invalidates them after membership loss or relinking', async () => {
    ...
    expect(description(await dispatch(component(token,coachDiscord),{channel_id:channelB}))).toContain('ne correspond pas');
    expect(await rows('select team_id,title from discord_team_goals')).toEqual([]);
    ...
    expect(description(await dispatch(component(obsolete,coachDiscord)))).toContain('pas accès');
