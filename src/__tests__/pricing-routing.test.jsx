...
vi.mock("../pages/admin/AudiencePage.jsx", () => ({ default: () => <section data-audience="true">Fréquentation</section> }));

const admin = { id: "admin", name: "Administrateur", email: "admin@example.test", email_verified: true, is_platform_admin: true };
const restrictedPaths = ["/tarifs", "/admin/demandes-acces", "/admin/frequentation"];
...
describe("pricing and access-request routes", () => {
  ...
  it("shows audience statistics to the platform administrator without a team", async () => {
    apiFetch.mockResolvedValue({ user: admin });
  ...
  it("releases the shared loader if workspace rendering fails without exposing diagnostics", async () => {
    const originalImplementation = useTeamData.getMockImplementation();
    const consoleError = vi.spyOn(console, "error").mockImplementation(() => {});
    useTeamData.mockImplementation(() => { throw new Error("private diagnostic details"); });
    apiFetch.mockResolvedValue({ user: admin });
    try {
      await open("/equipes");
      expect(renderer.root.findAllByProps({ "data-loader": "true" })).toHaveLength(0);
      const content = JSON.stringify(renderer.toJSON());
      expect(content).toContain("Ce contenu a rencontré un problème");
      expect(content).not.toContain("private diagnostic details");
      expect(renderer.root.findAllByProps({ role: "alert" })).toHaveLength(1);
    } finally {
      useTeamData.mockImplementation(originalImplementation);
