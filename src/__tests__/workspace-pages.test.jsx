...
describe("extracted workspace pages", () => {
  ...
  it("loads purchase events only when a champion game opens and refreshes them with bootstrap", async () => {
    ...
    act(() => renderer.root.findByProps({ className: "profile-champion-game" }).props.onToggle({ currentTarget: { open: true } }));
    ...
    expect(JSON.stringify(renderer.toJSON())).toContain("Chronologie des achats");
    ...
    expect(JSON.stringify(renderer.toJSON())).not.toContain("Chronologie des achats");
    ...
    act(() => renderer.root.findByProps({ className: "profile-champion-game" }).props.onToggle({ currentTarget: { open: false } }));
    act(() => renderer.root.findByProps({ className: "profile-champion-game" }).props.onToggle({ currentTarget: { open: true } }));
    expect(requests).toHaveLength(2);
  });
  it("can retry purchase details after a network failure", async () => {
    ...
    await act(async () => renderer.root.findByProps({ className: "profile-champion-game" }).props.onToggle({ currentTarget: { open: true } }));
