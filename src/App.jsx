...
function BrandLogo({ compact = false, className = "" }) {
  return (
    <div className={cx("flex items-center gap-3", className)}>
      <img
        src={compact ? "/riftboard-mark.png" : "/riftboard-logo.png"}
        alt="RiftBoard"
        className={cx(
...
function Topbar({ active, setOpen, refreshAll, loading }) {
  ...
  return <header className="sticky top-0 z-20 border-b border-white/10 bg-[#050711]/72 px-4 py-4 text-white backdrop-blur-2xl lg:px-8"><div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><button onClick={() => setOpen(true)} className="rounded-2xl border border-white/10 bg-white/[0.045] p-2 lg:hidden"><Menu className="h-5 w-5" /></button><div className={cx("hidden rounded-2xl border p-2.5 md:block", tone("cyan"))}><nav.icon className="h-5 w-5" /></div><div><p className="text-[0.68rem] font-black uppercase tracking-[0.26em] text-cyan-200/70">RiftBoard</p><h1 className="text-xl font-black tracking-tight md:text-2xl">{nav.label}</h1></div></div><div className="flex items-center gap-2"><Button variant="ghost" icon={loading ? Loader2 : RefreshCw} onClick={refreshAll} disabled={loading}>Sync DB</Button><Badge tone="green" pulse>Synced</Badge></div></div></header>;
}
