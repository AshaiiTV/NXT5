...
export default function DiscordWorkspace({ data, selectedTeamId, currentMember, user }) {
  ...
  return <div className="nxt5-data-dense discord-workspace">
    <PageHeader eyebrow="Avec ton équipe" title="Bot Discord" subtitle="Utilise NXT5 depuis Discord et partage les résultats de ton équipe." />
    <div className="mb-4"><DiscordAccount key={user?.id} user={user} /></div>
    {!team ? <Surface><EmptyState icon={Users} title="Choisis une équipe pour commencer" text="Crée ou rejoins une équipe NXT5, puis utilise les commandes dans son salon Discord une fois ton compte lié." action={<LinkButton href="/equipes" navigate={openAppPath} className="min-h-11">Ouvrir mes équipes</LinkButton>} /></Surface>
      : !canPublish ? <Surface><EmptyState icon={Shield} title="La connexion Discord se configure avec ton encadrement" text={`Le propriétaire ou un capitaine de ${team.name} associe le salon des commandes. Une fois ton compte lié, va dans ce salon Discord pour utiliser /nxt.`} /></Surface>
