...
export const LEGAL_PAGES = {
  ...
  "/confidentialite": {
    ...
    sections: [
      ...
      ["Durées de conservation", "Les informations du compte sont conservées jusqu’à sa suppression, dont les effets et les données résiduelles sont décrits ci-dessous. Les historiques et contenus partagés d’une équipe conservée restent disponibles jusqu’à la suppression de l’équipe ou du contenu concerné, sous réserve des demandes d’effacement applicables. Les sessions expirent après 12 heures, ou 30 jours lorsque « Rester connecté » est activé, puis leurs traces sont supprimées sous 30 jours. Les limites de tentative liées à l’IP sont purgées après 24 heures. Les journaux d’actions et le journal des destinataires des rappels d’inactivité sont conservés 12 mois ; ce dernier contient uniquement le compte, l’adresse utilisée, la période d’inactivité et la date d’envoi. Les liens de vérification et de réinitialisation expirent respectivement après 24 heures et 30 minutes, puis leurs données sont supprimées sous 30 jours. Les sauvegardes techniques de Neon peuvent subsister jusqu’à 30 jours après une suppression."],
      ["Suppression de ton compte", "Le bouton « Supprimer mon compte » est disponible dans Paramètres. Après les confirmations, la suppression efface le nom, le pseudonyme et l’e-mail du compte, invalide son mot de passe et ferme toutes ses sessions. Les profils joueurs directement liés, leurs pools de champions, disponibilités, objectifs et notes de coaching sont supprimés. Les historiques de matchs et contenus partagés des équipes conservées restent disponibles ; des mentions de ton pseudo peuvent subsister dans les imports et textes partagés. Utilise le canal privé de la page Contact pour demander l’examen des données personnelles qui y resteraient présentes ou si tu ne peux plus accéder au compte."],
      ["Traces de la suppression", "Un enregistrement technique désactivé du compte subsiste avec son identifiant et ses dates, sans son ancien nom ni son e-mail. Une preuve datée conserve une référence et le nombre d’éléments supprimés ou transférés, sans ton nom ni ton e-mail. Les preuves de plus de 12 mois sont purgées lors d’une nouvelle préparation de suppression de compte. Les demandes d’effacement portant sur ces traces peuvent être adressées par le canal privé de la page Contact."],
  ...
  "/conditions": {
    ...
    sections: [
      ...
      ["Fin d’utilisation", "Tu peux cesser d’utiliser NXT5 à tout moment et supprimer définitivement ton compte depuis Paramètres, dans la section « Supprimer mon compte ». Deux confirmations sont demandées, dont la saisie de SUPPRIMER et de ton mot de passe actuel. Si tu possèdes une équipe avec d’autres membres, tu dois désigner un nouveau propriétaire ; la suppression des équipes sans autre membre demande une confirmation spécifique. Après validation, ton accès est désactivé, toutes tes sessions sont fermées et un reçu daté est affiché. En cas de difficulté, le canal privé de la page Contact reste disponible."],
      ["Données après suppression du compte", "Les données personnelles directement liées au compte sont traitées selon les modalités de la Politique de confidentialité. Les historiques de matchs et contenus partagés des équipes conservées restent disponibles ; des mentions de ton pseudo peuvent subsister dans les imports ou les textes partagés. Tu peux en demander l’examen par le canal privé de la page Contact. Supprimer ton compte ne supprime pas automatiquement une équipe qui compte d’autres membres."],
  ...
  "/contact": {
    ...
    sections: [
      ...
      ["Supprimer ton compte", "Tu peux supprimer ton compte directement depuis Paramètres, avec le bouton « Supprimer mon compte ». Le parcours explique les conséquences et demande deux confirmations. Si tu ne peux plus te connecter ou si tu souhaites faire examiner des données restées dans des contenus partagés, contacte NXT5 en privé."],
...
export function AuthPage({ mode, onAuth, pushToast, navigate }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      if (!result.user?.is_platform_admin) void trackAudienceEvent(isRegister ? "signup" : "login");
