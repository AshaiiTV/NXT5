...
function validateStatus(data) {
  if (typeof data?.configured !== "boolean" || !Array.isArray(data.issues)) {
    throw new Error("La configuration Shopify est indisponible.");
  }
}

export default function IntegrationsPage({ navigate }) {
  const { data: status, loading, error: readError, refresh } = useAdminQuery("admin-shopify", { validate: validateStatus });
  const mutate = useAdminMutation();
  const [connection, setConnection] = useState(null);
  const [testing, setTesting] = useState(false);
  const [testError, setTestError] = useState("");
  const error = testError || readError;

  useEffect(() => {
    if (!status) setConnection(null);
  }, [status]);
  ...
  async function testConnection() {
    ...
    setTestError("");
    try {
      setConnection(await mutate("admin-shopify", { method: "POST" }));
    } catch (err) {
      ...
      <main className="relative z-10 mx-auto max-w-5xl space-y-6 px-5 pb-12 pt-8">
        ...
        <Surface glow>
          <div className="space-y-5 p-2 sm:p-4">
            ...
            {loading && <p role="status" className="text-sm">{status ? "Actualisation de la configuration…" : "Lecture de la configuration…"}</p>}
            ...
            {readError && status && <p className="text-sm leading-6 text-slate-300">La configuration ci-dessus date de la dernière lecture réussie.</p>}
            {connection && status && <div role="status" className="space-y-2 rounded-xl border border-cyan-200/25 bg-cyan-300/10 p-4 text-sm leading-6">
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <Button type="button" icon={RefreshCw} variant="ghost" disabled={loading || testing} onClick={() => { setTestError(""); setConnection(null); void refresh().catch(() => {}); }}>Actualiser</Button>
            </div>
          </div>
        </Surface>
        <LegalReadinessPage navigate={navigate} />
      </main>
...
export function LegalReadinessPage({ navigate }) {
  return <Surface>
          <div className="space-y-5 p-2 sm:p-4">
