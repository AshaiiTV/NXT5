  ...
  return <div>
    ...
    <div className="flex h-44 items-end gap-1 sm:gap-1.5" aria-label="Activité quotidienne sur 30 jours" onMouseLeave={() => setHovered(null)}>
      {points.map((row) => {
        ...
        return <button type="button" key={row.date} className="group relative flex h-full min-w-0 flex-1 items-end focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-200/80" aria-label={`${formatDate(row.date)} : ${total} événements`} onMouseEnter={() => setHovered(row)} onFocus={() => setHovered(row)} onClick={() => setHovered(row)}><span style={{ height: `${height}%` }} className="min-h-[3px] w-full rounded-t-sm bg-cyan-300 opacity-75 transition group-hover:opacity-100 group-focus-visible:opacity-100" /></button>;
      })}
    </div>
    <div className="mt-3 flex justify-between text-xs font-bold uppercase tracking-wider text-slate-500"><span>{formatDate(points[0]?.date)}</span><span>{formatDate(points.at(-1)?.date)}</span></div>
  </div>;
}

function TableShell({ children, label }) {
  return <div className="nxt5-responsive-scroll mt-4 rounded-xl border border-white/10" role="region" aria-label={label} tabIndex={0}><table className="w-full min-w-[720px] text-left text-sm">{children}</table></div>;
}
...
function ProgressRow({ label, value, total, suffix = "équipes" }) {
  ...
  return <div><div className="mb-1.5 flex flex-wrap items-center justify-between gap-x-3 gap-y-1 text-xs"><span className="font-semibold text-slate-300">{label}</span><strong className="text-white">{number.format(Number(value || 0))} <span className="font-semibold text-slate-500">{suffix} · {rate}%</span></strong></div><div className="h-2 overflow-hidden rounded-full bg-white/[0.07]"><div className="h-full rounded-full bg-cyan-300" style={{ width: `${Math.min(100, rate)}%` }} /></div></div>;
}

function WeeklyChart({ rows = [] }) {
  const max = Math.max(1, ...rows.map((row) => Number(row.matches || 0)));
  return <div className="mt-4 flex items-end gap-1.5" aria-label="Volume hebdomadaire des games">{rows.map((row) => <div key={row.date} className="flex min-w-0 flex-1 flex-col items-center gap-2"><span className="text-xs font-black tabular-nums text-slate-300">{row.matches || 0}</span><div className="h-24 w-full rounded-t-md bg-white/[0.05] flex items-end overflow-hidden" title={`${formatDate(row.date)} · ${row.matches || 0} games · ${row.activeUsers || 0} utilisateurs actifs`}><div className="w-full rounded-t-md bg-violet-300" style={{ height: `${Math.max(3, (Number(row.matches || 0) / max) * 100)}%` }} /></div></div>)}</div>;
}

function TeamMatchChart({ rows = [] }) {
  const max = Math.max(1, ...rows.map((row) => Number(row.matches || 0)));
  return <div className="flex h-24 items-end gap-1" aria-label="Imports de cette équipe sur 30 jours">{rows.map((row) => <div key={row.date} className="min-w-0 flex-1 rounded-t-sm bg-violet-300" title={`${formatDate(row.date)} · ${row.matches || 0} games`} style={{ height: `${Math.max(3, (Number(row.matches || 0) / max) * 100)}%` }} />)}</div>;
}

function BenchmarkMetric({ label, value, median, suffix = "" }) {
  const delta = Number(value || 0) - Number(median || 0);
  return <div className="min-w-0 border-l border-white/15 pl-3 py-1"><p className="text-xs font-black uppercase tracking-wider text-slate-400">{label}</p><div className="mt-2 flex flex-wrap items-end justify-between gap-2"><strong className="text-2xl text-white">{number.format(Number(value || 0))}{suffix}</strong><span className={cx("text-xs font-black", delta > 0 ? "text-emerald-200" : delta < 0 ? "text-amber-200" : "text-slate-400")}>{delta > 0 ? "+" : ""}{decimal.format(delta)} vs médiane</span></div></div>;
}
...
export default function AdminDashboard({ navigate }) {
  ...
  return <div className="nxt5-data-dense min-w-0">
    ...
    <Surface className="mt-4">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-fuchsia-100/70">Analyse par équipe</p><h3 className="mt-1 text-2xl font-black text-white">Données détaillées et anonymisées</h3><p className="mt-2 text-sm font-semibold text-slate-400">Volumes et usage produit sans afficher les joueurs, leurs identifiants ou les contenus du staff.</p></div><div className="grid w-full gap-2 sm:grid-cols-[1fr_1.2fr] lg:max-w-2xl"><TextInput label="Rechercher une équipe" value={teamSearch} onChange={setTeamSearch} placeholder="Nom, tag ou région" icon={Search} /><SelectInput label="Équipe à analyser" value={selectedTeamId} onChange={setSelectedTeamId}>{!filteredTeams.some((team) => team.id === selectedTeamId) && <option value={selectedTeamId}>{teamDetail?.team?.name || "Sélection actuelle"}{filteredTeams.length ? " · hors recherche" : " · aucune équipe trouvée"}</option>}{filteredTeams.map((team) => <option key={team.id} value={team.id}>{team.name} [{team.tag}] · {team.players} joueurs · {team.matches} games</option>)}</SelectInput></div></div>
      {teamLoading ? <div className="py-8"><SkeletonRows count={3} /></div> : teamError ? <div className="mt-5 rounded-xl border border-rose-300/25 bg-rose-500/10 p-4 text-sm font-semibold text-rose-100">{teamError}</div> : teamDetail && <div className="mt-6 border-t border-white/10 pt-5">
        ...
        <div className="mt-4 grid gap-4 xl:grid-cols-[1.1fr_.9fr]">
          <div className="min-w-0 border-t border-white/10 pt-4"><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wider text-slate-400">Rythme réel</p><p className="mt-1 text-sm font-semibold text-slate-300">Imports des 30 derniers jours</p></div><div className="text-right"><p className="text-2xl font-black text-white">{teamMatches.last30d || 0}</p><p className="text-xs font-semibold text-slate-500">{teamMatches.last7d || 0} cette semaine</p></div></div><div className="mt-4"><TeamMatchChart rows={teamDetail.daily || []} /></div></div>
          <div className="min-w-0 border-t border-white/10 pt-4"><p className="text-xs font-black uppercase tracking-wider text-slate-400">Points à vérifier</p><div className="mt-3 space-y-2">{(teamDetail.health?.signals || []).length ? teamDetail.health.signals.map((signal) => <div key={signal.title} className={cx("rounded-xl border p-3", signal.level === "critical" ? "border-rose-300/20 bg-rose-400/[0.07]" : signal.level === "warning" ? "border-amber-300/20 bg-amber-400/[0.07]" : signal.level === "positive" ? "border-emerald-300/20 bg-emerald-400/[0.07]" : "border-cyan-300/20 bg-cyan-400/[0.07]")}><p className="text-sm font-black text-white">{signal.title}</p><p className="mt-1 text-xs font-semibold leading-5 text-slate-300">{signal.detail}</p></div>) : <p className="text-sm font-semibold text-slate-400">Aucun point particulier pour le moment.</p>}</div></div>
        </div>
        <div className="mt-4 grid gap-4 lg:grid-cols-2"><div className="min-w-0 border-t border-white/10 pt-4"><p className="text-xs font-black uppercase tracking-wider text-slate-400">Complétude opérationnelle</p><div className="mt-4 space-y-3"><ProgressRow label="Rôles titulaires couverts" value={teamTotals.mainRolesCovered} total={5} suffix="rôles" /><ProgressRow label="Profils liés" value={teamTotals.linkedPlayers} total={teamPlayers} suffix="joueurs" /><ProgressRow label="Patch renseigné" value={teamMatches.withPatch} total={Number(teamTotals.matches || 0)} suffix="games" /><ProgressRow label="Durée exploitable" value={teamMatches.withDuration} total={Number(teamTotals.matches || 0)} suffix="games" /></div></div><div className="min-w-0 border-t border-white/10 pt-4"><p className="text-xs font-black uppercase tracking-wider text-slate-400">Workflow staff</p><div className="mt-3 grid grid-cols-2 gap-2">{[["Reviews",teamTotals.reports,teamDetail.benchmark?.medianReports],["Compositions",teamTotals.compositions,teamDetail.benchmark?.medianCompositions],["Joueurs planifiés",teamTotals.playersPlannedCurrentWeek,null],["Objectifs",teamTotals.goals,null]].map(([label,value,median]) => <div key={label} className="min-w-0 border-l border-white/15 py-1 pl-3"><strong className="text-xl text-white">{value || 0}</strong><p className="mt-1 text-xs font-bold uppercase text-slate-400">{label}</p>{median !== null && <p className="mt-1 text-xs font-semibold text-cyan-200">Médiane : {decimal.format(Number(median || 0))}</p>}</div>)}</div></div></div>
