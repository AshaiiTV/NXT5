...
function PopularPages({ rows, total }) {
  ...
  return <Section title="Pages consultées" icon={MousePointer2} description="Vues, navigateurs distincts et temps actif. Les chemins sont normalisés pour protéger les données personnelles." action={<Badge tone="cyan">{n(rows.length)} pages</Badge>}>
    <div className="audience-table-toolbar"><div className="audience-search-field"><TextInput label="Rechercher une page" icon={Search} type="search" placeholder="Rechercher un chemin…" value={search} onChange={(value) => { setSearch(value); setPage(1); }} /></div><SelectInput label="Trier par" aria-label="Trier les pages" value={sort} onChange={(value) => { setSort(value); setPage(1); }}><option value="views">Pages vues</option><option value="visitors">Navigateurs</option><option value="avgDurationSeconds">Temps actif</option><option value="exits">Sorties</option><option value="path">Chemin A → Z</option></SelectInput></div>
    {filtered.length ? <div className="audience-table-scroll nxt5-responsive-scroll" tabIndex={0} role="region" aria-label="Classement des pages"><table className="audience-table audience-pages-table"><thead><tr><th scope="col">Page</th><th scope="col">Vues</th><th scope="col">Navigateurs</th><th scope="col">Temps actif / vue</th><th scope="col">Sorties</th></tr></thead><tbody>{filtered.slice((safePage - 1) * 10, safePage * 10).map((row) => <tr key={row.path}><th scope="row"><span className="audience-page-path">{row.path}</span><span className="audience-page-bar" aria-hidden="true"><span style={{ width: `${audienceShare(row.views, total)}%` }} /></span></th><td><strong>{n(row.views)}</strong><small>{percent(audienceShare(row.views, total))}</small></td><td>{n(row.visitors)}</td><td>{duration(row.avgDurationSeconds)}</td><td>{n(row.exits)}</td></tr>)}</tbody></table></div> : <Note>{rows.length ? "Aucune page ne correspond à cette recherche." : "Aucune page vue sur cette période."}{search && <button className="audience-text-button" type="button" onClick={() => { setSearch(""); setPage(1); }}>Effacer la recherche</button>}</Note>}
...
export function AudienceHeatmap({ rows = [] }) {
  ...
  return <Section title="Quand le site est-il consulté ?" icon={CalendarDays} description="Pages vues cumulées par jour de la semaine et heure UTC sur la période.">
    ...
    <div className="audience-heatmap-scroll nxt5-responsive-scroll" role="region" aria-label="Consultations par jour et par heure" tabIndex={0}><div className="audience-heatmap" role="group" aria-label="Carte des consultations par heure. Flèches pour se déplacer."><span />{Array.from({ length: 24 }, (_, hour) => <span className="audience-heatmap-hour" key={`hour-${hour}`}>{hour % 3 === 0 ? `${hour} h` : ""}</span>)}{WEEK.map(([weekday, label], dayIndex) => <React.Fragment key={weekday}><span className="audience-heatmap-day">{label.slice(0, 3)}.</span>{Array.from({ length: 24 }, (_, hour) => {
...
export default function AudiencePage({ navigate }) {
  ...
  const key = `${days}|${device}|${source}`;
  const report = snapshot?.key === key ? snapshot.data : null;
  useEffect(() => {
    let active = true;
    const controller = new AbortController();
    setLoading(true); setError(null); setExportError("");
    const params = new URLSearchParams({ days: String(days), device, source });
    apiFetch(`admin-audience?${params}`, { signal: controller.signal, timeoutMs: 25000 })
      .then((data) => { if (!active) return; if (!isAudienceReport(data)) throw new Error("La réponse des statistiques est incomplète. Réessaie dans quelques instants."); setSnapshot({ key, data }); })
      .catch((cause) => { if (active) setError({ key, message: cause.message || "Impossible de charger la fréquentation.", code: cause.code }); })
      .finally(() => { if (active) setLoading(false); });
    return () => { active = false; controller.abort(); };
  }, [days, device, source, revision, key]);
  useEffect(() => {
    if (!autoRefresh) return undefined;
    const timer = globalThis.setInterval(() => { if (typeof document === "undefined" || document.visibilityState !== "hidden") setRevision((value) => value + 1); }, 60000);
    return () => globalThis.clearInterval(timer);
  }, [autoRefresh]);
  const sources = [...new Set([...(snapshot?.data?.filters?.sources || []), ...(source !== "all" ? [source] : [])])].filter((value) => value !== "all").sort();
  ...
  return <div className="nxt5-data-dense audience-dashboard">
    <PageHeader eyebrow="Administration · Audience" title="Fréquentation" subtitle="Suis les visites, les pages consultées et les actions enregistrées sur la période choisie.">
      ...
      <Button type="button" variant="ghost" icon={loading ? Loader2 : RefreshCw} disabled={loading} onClick={() => setRevision((value) => value + 1)}>{loading ? "Actualisation…" : "Actualiser"}</Button>
    </PageHeader>
