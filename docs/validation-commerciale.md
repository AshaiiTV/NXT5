...
- La grille, le formulaire et les choix manuels reprennent uniquement Découverte et le Pass Équipe avec les mêmes libellés tarifaires. Le lien pour plusieurs équipes et le choix Structure sont retirés. Pass Saison, Pass Structure, annuel et fondateur ne sont plus proposés. Les anciennes demandes et leurs codes restent consultables dans le suivi, sans conversion ni perte de leur historique.
...
5. Avec le compte administrateur configuré, vérifier l’accès aux deux pages et aux deux cartes de lancement. Tester la sélection de Découverte puis du Pass Équipe et vérifier que le formulaire et les choix manuels reprennent exactement ces deux offres. Vérifier la durée de 14 jours, l’accès complet jusqu’à 15 membres, le prix de 9,90 € TTC/mois/équipe et l’absence d’anciennes cartes, de lien multi-équipe ou de choix Structure. Vérifier la validation des champs et la confirmation d’enregistrement de demandes fictives ; les anciennes demandes Saison et Structure restent consultables. Tester filtre, pagination, note, statut et suppression. Aucun de ces parcours commerciaux ne doit démarrer un essai ou un paiement. Vérifier également le rendu mobile.
...
Pour une nouvelle demande, le champ `planCode` accepte uniquement `free` et `team_monthly`, les deux offres de Tarifs. Les codes `team_season` et `structure` sont refusés à la création, y compris par appel direct à l’API ; les demandes déjà enregistrées avec ces codes restent lisibles et conservent leur suivi. Le code `free` identifie la proposition d’essai de 14 jours, sans le démarrer. L’administration des abonnements de profils accepte uniquement `free` et `team_monthly`. La migration des anciennes attributions Saison et Structure en Pass Équipe ne modifie aucune demande commerciale et ne la convertit pas en abonnement ou essai.
...
Les éventuelles demandes historiques concernant plusieurs équipes conservent leurs notes et leur suivi. Elles ne valident pas le tarif du Pass Équipe et n’accordent aucun droit produit. Le parcours actuel ne recueille plus de nouvelles demandes Structure ; une offre éventuelle fera l’objet d’une décision ultérieure.
