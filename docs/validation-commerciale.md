...
5. Avec le compte administrateur configuré, vérifier l’accès aux deux pages et les quatre cartes. Tester la sélection du Pass Saison puis du Pass Structure, la validation des champs et la confirmation d’enregistrement de demandes fictives. Vérifier que Structure est conservé comme offre choisie dans le suivi, puis tester filtre, pagination, note, statut et suppression. Une demande Structure ne doit ouvrir ni paiement ni fonctionnalité multi-équipe.
...
Pour une demande Pass Structure, préciser aussi le nombre d’équipes concernées, les besoins de coordination et de facturation, le responsable de la structure et l’accompagnement attendu. Le montant « à partir de 79 € TTC par mois » est un point de départ à qualifier sur devis, pas un prix définitif accepté. Consigner ces besoins dans les notes existantes sans promettre de fonctions disponibles ni de quotas non définis.
...
| Intention confirmée après échange | L’équipe a confirmé son intérêt pour la formule et le prix, et identifié qui paierait. Pour Structure, le périmètre, le devis et le payeur doivent avoir été acceptés ; le tarif de départ ne suffit pas. Ce statut n’active aucun droit. |
| Offre présentée, sans suite | L’échange a eu lieu et l’équipe ne poursuit pas ; noter pourquoi. |

Les objectifs utilisent des équipes ou organisations distinctes d’après leur nom normalisé. Les contacts multiples d’une même équipe ou structure ne sont pas additionnés ; deux organisations qui utilisent exactement le même nom seront regroupées et doivent être vérifiées manuellement. Seules les confirmations manuelles d’offres payantes alimentent l’objectif des trois intentions d’achat. Une demande Structure est d’abord un recueil de besoins et ne compte pas automatiquement : après validation du périmètre, du devis et du payeur, sa confirmation compte pour une organisation, jamais pour le nombre d’équipes qu’elle représente.

## Quand passer au paiement

Attendre dix présentations et au moins trois intentions confirmées d’équipes ou organisations distinctes pour une offre payante, avec formule, prix et payeur documentés ; pour Structure, documenter aussi le périmètre et le devis accepté. Si ces critères ne sont pas remplis, utiliser les réserves recueillies pour revoir l’offre.
