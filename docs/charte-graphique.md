...
| Emblème compact complet | `public/assets/nxt5-loader-favicon.png` et `nxt5-loader-favicon-256.webp` ; conserver les trois branches du trident. |
...
**Correction explicite du 23 septembre 2026 :** ne plus utiliser `public/assets/nxt5-mark.png` ni `nxt5-mark-160.webp` : leur trident est incomplet. Utiliser le logo complet ou `nxt5-loader-favicon.png` et sa variante WebP, après contrôle des trois branches. Le fichier homonyme de l’Importer est distinct et complet ; cette interdiction ne le concerne pas. Cette correction prime sur toute référence historique ci-dessous.
...
- **Étapes sélectionnables :** présenter quatre catégories cliquables, dans cet ordre : **Inviter, Relier, Choisir les salons, Tester et activer**. Un clic affiche uniquement le contenu de l’étape sélectionnée sous cette navigation. La sélection est distincte de l’avancement réel : ouvrir une étape ne valide ni invitation, ni liaison, ni test, ni activation. Les catégories restent visibles après l’activation. Par défaut, sélectionner Inviter sans liaison et Choisir les salons lorsqu’un serveur est relié ; un rafraîchissement ne remplace pas le choix manuel. Une étape dont les prérequis manquent explique ce qui reste à faire et garde ses actions indisponibles.
- **Clavier et continuité :** utiliser une navigation d’onglets nommée avec `tablist`, `tab`, `aria-selected`, `aria-controls` et un `tabpanel` associé. Un seul onglet entre dans l’ordre de tabulation ; les flèches déplacent la sélection, Début et Fin rejoignent les extrémités. Le focus reste visible. Changer d’étape conserve le code et son expiration, les brouillons, le reçu de test et les états des requêtes de la même équipe ; cela ne déclenche aucun envoi. Les panneaux masqués ne sont ni visibles ni accessibles au clavier. Le changement d’équipe réinitialise ce contexte.
...
- **Destinations :** au début de « Destinations des games », rendre directement visibles pour les responsables le sélecteur natif « Choisir un salon du serveur », « Ajouter ce salon » et « Actualiser les salons ». Le choix initial reste vide ; ne jamais présélectionner le premier salon. Afficher les noms `#salon` du serveur relié et désactiver les salons sans autorisation d’envoi ou déjà ajoutés à cette équipe. Expliquer chargement, erreur, absence de salon utilisable et limite de dix destinations. L’ajout prépare un brouillon sans enregistrement ni envoi ; les catégories et options de mention se règlent ensuite dans la destination. « Enregistrer les destinations » reste une action distincte du test et de l’activation. Un rafraîchissement conserve le brouillon. Tant qu’il n’est pas enregistré, le test et l’activation sont désactivés avec une explication. Conserver les restrictions de rôle, le label persistant et le focus visible ; sur mobile, empiler le sélecteur et ses actions sans débordement.
...
- **Après la connexion :** donner la priorité à l’équipe active, au serveur, à l’état de diffusion et aux actions utiles. Nommer l’équipe concernée par les salons, catégories, mentions, tests, pause, reprise et déconnexion. Sur un serveur partagé, ces actions restent propres à cette équipe et ne modifient pas les autres liaisons ; une déconnexion ne retire pas le bot du serveur. Regrouper destinations, dernière publication et historique dans le même espace de gestion. La dernière publication donne accès au message Discord lorsqu’un lien existe ; une absence d’envoi reste un état vide, sans exemple présenté comme une publication réelle. Pause et reprise restent explicites.
- **Aide persistante :** les aides à la connexion et à la résolution des problèmes restent accessibles sous le suivi. Les quatre onglets restent toujours visibles, y compris après activation, et le contenu de test apparaît directement dans l’étape 4, sans seconde aide à ouvrir. Expliquer permissions, visibilité du contenu et envois incertains sans afficher de jeton, de secret ou de réglage réservé à l’opérateur du bot.
...
## Connexions Google, Discord, Apple et Riot — 23 septembre 2026

L’inscription et la connexion peuvent proposer Google, Discord, Apple et Riot lorsqu’ils sont activés. Présenter des boutons lisibles, avec le nom du service en entier, au-dessus du formulaire e-mail existant. Aucun bouton ne lance un service non configuré. Conserver la palette et les contrôles partagés ; ne pas transformer le logo NXT5 en icône d’un fournisseur. Les boutons Google et Apple respectent leurs marques officielles (fond blanc, glyphes et police autorisés) dans le panneau NXT5 ; cette exception concerne uniquement ces actions d’authentification.

Après une première authentification externe, demander le pseudo, l’adresse de récupération et l’acceptation des textes en vigueur avant de créer le compte. Aucune fusion de comptes ne se fait sur la seule correspondance d’e-mail. Les messages expliquent comment revenir au compte existant et associer un service depuis Paramètres.

Dans Paramètres, regrouper les quatre services dans une seule surface « Connexions associées ». Afficher association, disponibilité, chargement et erreur à partir des réponses serveur. La dissociation se confirme avec le mot de passe NXT5, pour conserver un accès utilisable. Un compte sans mot de passe peut en définir un via un lien reçu par e-mail ; expliquer que cette récupération termine les sessions et retire les anciennes associations.

Sur mobile, conserver les noms et les actions entiers, permettre le retour à la ligne et éviter tout débordement à 320 px. Les confirmations restent accessibles au clavier ; les erreurs sont annoncées et le focus retourne à l’action correspondante. Les textes légaux restent accessibles durant l’inscription.
