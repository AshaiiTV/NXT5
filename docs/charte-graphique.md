...
La fiche d’une game réutilise le même modèle de données et les mêmes opérations de dessin dans l’export du site et le traitement serveur. La largeur est de **1 200 px** ; la hauteur s’adapte aux noms et aux données disponibles. Les autres exports existants conservent leurs formats propres.
...
| Hiérarchie | Équipes et catégorie, résultat/date/durée, quatre repères, lecture et piste de review autorisées, compositions, couverture des données. |
| Côtés et données | Côté bleu à gauche et rouge à droite, avec l’appartenance à notre équipe explicite. Un écart conserve son unité et son sens ; une mesure absente reste indisponible. |

Conserver les retours à la ligne pour les noms longs et la couverture propre aux données. Une absence de timeline n’est pas une valeur nulle. Les observations ne deviennent pas des verdicts du coach ; les notes privées du staff ne sont pas incluses automatiquement. Le PNG doit aussi rester compréhensible quand les pistes de review ne sont pas autorisées pour le salon.
