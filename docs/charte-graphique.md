...
- **Filtres communs :** « Contexte des games » et « Période d’analyse » s’appliquent aux cinq rubriques. La période propose toutes les games ou les 5, 10, 20 dernières, classées par date de jeu. Montrer le nombre de games analysées, le contexte actif, la réinitialisation et le message de petit échantillon lorsque nécessaire.
...
Évolution autorisée le 15 septembre 2026 : les profils concentrent la lecture sur un joueur, un contexte de games et une rubrique à la fois. La direction artistique reste celle de NXT5 : fond nuit, surfaces calmes et accents cyan/violet/fuchsia. Les boutons et onglets gardent leur rayon de 2 px (`--nxt5-radius-button`) et les champs leur rayon de 12 px (`--nxt5-radius-control`). L’identité du joueur apparaît une seule fois, avec les sélections persistantes « Joueur » et « Contexte ».
