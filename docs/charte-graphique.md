...
| --- | --- |
| Synthèse | Quatre chiffres avec unités et contexte : taux de victoire, écart d’or, morts de l’équipe, premier objectif. Viennent ensuite l’axe de travail proposé et sa cible, le plan de jeu récurrent et les contributions par rôle, les points à vérifier en review, puis les résultats par côté. |
| Évolution | Comparaison de deux blocs consécutifs de même taille : périodes datées, valeurs récente et précédente, variation signée, couverture des données et accès aux games sources. La série récente reste lisible dans l’ordre chronologique. |
| Comparer | Deux choix explicites « Bloc de référence » et « Bloc observé », avec dates, nombres de games et commande d’inversion. L’évolution vaut « observé − référence ». Les résultats d’équipe, la participation aux kills par rôle et les taux de victoire par côté conservent unités, échantillons et états indisponibles. Signaler les games communes et les blocs vides. |
...
- **Hiérarchie :** organiser avec espaces, titres et séparateurs. Les contributions par rôle se lisent en lignes avec `RoleIcon`, champions, interprétation, parts d’or et de dégâts, puis sources. Les points de review utilisent `details` / `summary`, repliés par défaut ; le constat, les preuves et l’action restent accessibles au clavier.
